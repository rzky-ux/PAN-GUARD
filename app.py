...
with tab3:
    ...
    # Hilangkan nilai kosong agar tidak memicu deteksi peringatan palsu akibat data null
    df_ewis = df_filtered.dropna(subset=["Curah Hujan (mm)", "Suhu Rata-rata (°C)", "NTP"])
    
    # Menentukan threshold / ambang batas bahaya
    kondisi_kritis = df_ewis[
        (df_ewis["Curah Hujan (mm)"] > 300) | 
        (df_ewis["Suhu Rata-rata (°C)"] > 28.5) | 
        (df_ewis["NTP"] < 100)
    ]
